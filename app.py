...
def show_feedback():
    ...
    st.info(
        """Thank you for completing the interview. Please provide your feedback on the interview experience.
            """,
            icon="🙏"
            )
...
if st.session_state.setup_complete and not st.session_state.feedback_shown and not st.session_state.chat_complete:
    ...
    if  not st.session_state.messages:
        st.session_state.messages = [
            {"role": "system", 
             "content": f"You are a HR executive that interviews an inteviewee called {st.session_state['name']} "
...
if st.session_state.feedback_shown:
    ...
    feedback_completion = feedback_client.chat.completions.create(
        model="gpt-4o",
        messages=[
                {"role": "system", "content": """You are a helpfull tool that provides feedback on an interviewee performance.
                 ...
                 """},
                {"role": "user", "content": f"This is the interview you need to evaluate. Keep in mind that you are only a tool. And you should "}
            ] # type: ignore
    ) # type: ignore    
